...
if(jQuery('.remove').length > 0) {
	jQuery('.remove').on('click', function () {
		console.log(jQuery('.cart_item').length);
...
function copyAddressToCheckout(){
	var strAddress=jQuery("#hdnAvailableAddress").val();
	var objAddress=JSON.parse(strAddress);
	jQuery("#billing_first_name").val(objAddress.billing_first_name);
	jQuery("#billing_last_name").val(objAddress.billing_last_name);
	jQuery("#billing_email").val(objAddress.billing_email);
	jQuery("#billing_mobile").val(objAddress.billing_mobile);
	jQuery("#billing_phone").val(objAddress.billing_phone);
	jQuery("#billing_address_1").val(objAddress.billing_address_1);
	jQuery("#billing_address_2").val(objAddress.billing_address_2);
	jQuery("#billing_country").val(objAddress.billing_country);
	jQuery('#billing_country').trigger('change');
	jQuery("#billing_city").val(objAddress.billing_city);
	jQuery("#billing_state").val(objAddress.billing_state);
	jQuery('#billing_state').trigger('change');
	jQuery("#billing_postcode").val(objAddress.billing_postcode);
	jQuery("#billing_state").val(objAddress.billing_state);
}
